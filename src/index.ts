...
async function main(): Promise<void> {
  ...
  initLlmFromFile();
  const transportId = resolveTransportId();
  const runner = createTransportRunner(transportId);
  console.log(`🚚 Transporte activo: ${runner.id}\n`);
  ...
  while (true) {
    ...
    await runner.run(handleMessage, { authMode: mode, phoneNumber });
    ...
    try {
      rmSync(runner.getAuthDir(), { recursive: true, force: true });
    } catch {
