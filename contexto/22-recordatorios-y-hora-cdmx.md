...
> Estado actual: la persistencia global descrita originalmente fue sustituida por `persistent/contexts/<jid>/reminders.json`; consulta `contexto/54-recordatorios-por-sandbox-y-contexto-secuencial.md`.
...
| Archivo | Descripción |
|---|---|
| `src/reminder.ts` | `ReminderManager` con persistencia actual en `persistent/contexts/<jid>/reminders.json`. Tool definition `create_reminder`. `executeReminderTool()`. Verificador periódico cada 30s con ventana `lastCheckTs` (epoch min-of-day + date) que tolera hasta 2 min de retraso sin disparar viejos. |
...
- **Recordatorios**: `create_reminder` tool acepta `text`, `hour`, `minute`, `date` opcional. Si no hay fecha, asigna hoy si la hora no pasó, mañana si ya pasó. Actualmente se guarda en `persistent/contexts/<jid>/reminders.json`.
