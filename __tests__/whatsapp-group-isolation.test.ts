...
import { isWhatsAppGroupJid } from "../src/transports/baileys/adapter.ts";
...
describe("aislamiento de grupos de WhatsApp", () => {
  ...
  it("rechaza login directo desde un JID de grupo histórico", async () => {
    ...
    try {
      ...
      const loggedIn = await auth.login("120363123456789@g.us", "admin", "pass1234");
  ...
  it("descarta sesiones de grupo antiguas al cargar users.json", async () => {
    ...
    try {
      ...
      const data = JSON.parse(readFileSync(path, "utf8")) as { users: unknown[]; sessions: Record<string, string> };
      ...
      const sanitized = JSON.parse(readFileSync(path, "utf8")) as { sessions?: Record<string, string> };
  ...
  it("filtra grupos dentro del runner Baileys antes del núcleo", async () => {
    const source = await Bun.file(new URL("../src/transports/baileys/runner.ts", import.meta.url)).text();
    expect(source).toContain("if (isWhatsAppGroupJid(remoteJid)) continue;");
  });

  it("mantiene una segunda barrera genérica antes de marcar leído", async () => {
    const source = await Bun.file(new URL("../src/bot.ts", import.meta.url)).text();
    const guardIndex = source.indexOf("if (!remoteJid || message.fromSelf || message.isGroup)");
    const readIndex = source.indexOf("transport.markRead(message)", guardIndex);
